...
| **Homebrew tap** | macOS, Linux | GoReleaser `brews` block writes a formula | new `orlandoburli/homebrew-tap` repo |
...
| **`go install`** | all (with Go) | already works via module path | n/a — document only |
