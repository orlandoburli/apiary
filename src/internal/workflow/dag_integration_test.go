...
func TestDAGIntegration_SequentialMemory(t *testing.T) {
	...
	instID, success, err := eng.RunInstance(ctx, wf, model.Cell{ID: "T-1", Title: "Add feature"})
	if err != nil {
...
func TestDAGIntegration_DiamondFanIn(t *testing.T) {
	...
	instID, success, err := eng.RunInstance(ctx, wf, model.Cell{ID: "T-2"})
	if err != nil {
		t.Fatalf("RunInstance: %v", err)
	}
	if !success {
...
func TestDAGIntegration_SplitByMemoryField(t *testing.T) {
	...
	instID, success, err := eng.RunInstance(ctx, wf, model.Cell{ID: "T-3"})
	if err != nil {
		t.Fatalf("RunInstance: %v", err)
	}
	if !success {
...
func TestDAGIntegration_OnFailLoopBackAndRetry(t *testing.T) {
	...
	instID, success, err := eng.RunInstance(ctx, wf, model.Cell{ID: "T-4"})
	if err != nil {
		t.Fatalf("RunInstance: %v", err)
	}
	if !success {
...
func TestDAGIntegration_OnFailMaxRetriesExhausted(t *testing.T) {
	...
	instID, success, err := eng.RunInstance(ctx, wf, model.Cell{ID: "T-5"})
	if err != nil {
		t.Fatalf("RunInstance: %v", err)
	}
	if success {
