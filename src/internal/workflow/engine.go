...
// and foreach worker goroutines. publishAllowList is the workflow-level
// publish_allow list: when non-empty, only listed step IDs may write back via
// APIARY_PUBLISH; steps absent from the list have their payload suppressed.
func (e *Engine) runStep(ctx context.Context, instID string, step config.StepConfig, cell model.SourceItem, task model.InternalTask, bindings []model.SourceBinding, memSteps []MemoryStep, wfEnv map[string]string, publishAllowList []string) StepResult {
	started := e.now()
	sr := &db.StepRun{
		ID:                 e.newID("sr"),
		WorkflowInstanceID: instID,
	...
	// Workflow-level publish allow-list: suppress the payload when the list is
	// non-empty and this step is not listed. This mirrors publish: off without
	// requiring each step to declare it individually.
	if len(publishAllowList) > 0 && !publishAllowed(step.ID, publishAllowList) {
		res.PublishPayload = ""
	}
...
// publishAllowed reports whether stepID is in the allow-list.
func publishAllowed(stepID string, list []string) bool {
	for _, id := range list {
		if id == stepID {
			return true
		}
...
func (e *Engine) spawnStep(ctx context.Context, task model.InternalTask, step config.StepConfig, bindings []model.SourceBinding, res *StepResult, sr *db.StepRun) {
	...
	for _, req := range reqs {
		...
		req.ParentSpawnDepth = task.Metadata.SpawnDepth
...
// agentAuthoredFooter is appended to every APIARY_PUBLISH payload so readers
// and audit tools can distinguish agent-authored comments from human ones.
const agentAuthoredFooter = "\n\n---\n*This comment was generated by an AI agent (apiary). It should be reviewed before acting on it.*"
...
// spawned task) is silently skipped. A provenance footer is appended to every
// payload so the comment is identifiable as agent-authored in the source UI.
func (e *Engine) publishStep(ctx context.Context, task model.InternalTask, bindings []model.SourceBinding, res StepResult, sr *db.StepRun) {
	...
	payload := res.PublishPayload + agentAuthoredFooter
	sr.PublishPayload = payload
